...
from .utils import get_system_metrics, sanitize_clinical_text, apply_hybrid_pipeline
...
MODEL_PREDICTIONS_TOTAL = Counter('model_predictions_total', 'Total model predictions', ['label'])
...
app.add_middleware(SecurityHeadersMiddleware, csp_policy="default-src 'self'; script-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net; style-src 'self' https://cdn.jsdelivr.net; img-src 'self' https://fastapi.tiangolo.com")
...
async def predict_assertion(request: PredictionRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        logger.info(f"Prediction request {request_id}: sentence_length={len(request.sentence)}")
        ...
        logger.info(f"Prediction completed {request_id}: final_label={result['label']}, model_label={result['model_label']}, rule={result.get('rule_applied')}, time={prediction_time:.3f}s")
...
async def predict_batch(request: BatchPredictionRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        logger.info(f"Batch prediction {request_id}: batch_size={len(request.sentences)}")
        ...
        logger.info(f"Batch prediction completed {request_id}: batch_size={len(enhanced_results)}, time={prediction_time:.3f}s")
